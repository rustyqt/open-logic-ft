        ...
        while test_suite loop
            ...
            if run("Encode-NoFlip") then
                ...
                for i in 0 to CodewordWidth_c - 1 loop
                    applyAndCheck(toUslv(16#A5#, Width_g), singleBit(i),
                        "single flip at " & integer'image(i));
                end loop;

            elsif run("Encode-WithDoubleFlip") then
                applyAndCheck(toUslv(16#5A#, Width_g), doubleBit(0, 1),                                       "(0,1)");
                applyAndCheck(toUslv(16#5A#, Width_g), doubleBit(0, CodewordWidth_c - 1),                     "(0,N-1)");
                applyAndCheck(toUslv(16#5A#, Width_g), doubleBit(1, 2),                                       "(1,2)");
                applyAndCheck(toUslv(16#5A#, Width_g), doubleBit(2, 5),                                       "(2,5)");
                applyAndCheck(toUslv(16#5A#, Width_g), doubleBit(CodewordWidth_c / 2, CodewordWidth_c / 2 + 1), "(mid,mid+1)");
