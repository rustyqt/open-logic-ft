...
architecture sim of olo_ft_ram_sp_scrub_tb is
    ...
    procedure read_word (
        ...
        signal   RdEccSec   : in  std_logic;
        signal   RdEccDed   : in  std_logic) is
    begin
        wait until rising_edge(Clk);
        Addr <= toUslv(address, Addr'length);
        ...
        check_equal(RdEccSec, exp_secerr, msg & " EccSec");
        check_equal(RdEccDed, exp_dederr, msg & " EccDed");
    end procedure;
    ...
    begin
        ...
        while test_suite loop
            ...
            -- Scrubber finds and fixes a single-bit error
                ...
                Scrub_Stop <= '1';
                wait until Scrub_Stopped = '1' and rising_edge(Clk);
                read_word(5, 16#AB#, '0', '0', "After scrub addr5", Clk, Addr, RdData, RdEccSec, RdEccDed);
            ...
            -- Scrubber detects double-bit error
                ...
                read_word(7, 16#CD#, '0', '1', "Ded after scrub", Clk, Addr, RdData, RdEccSec, RdEccDed);
            ...
            -- Note: data is unreliable on DED, so this only verifies the EccSec/EccDed flags.
                ...
                for pair in 0 to 4 loop
                    ...
                    check_equal(RdEccSec, '0', "DedPair " & integer'image(pair) & " EccSec");
                    check_equal(RdEccDed, '1', "DedPair " & integer'image(pair) & " EccDed");
                end loop;
                ...
                for pass in 1 to 3 loop
                    ...
                    for i in 0 to Depth_c - 1 loop
                        read_word(i, i * 3 + 1, '0', '0', "Interleaved pass " & integer'image(pass) &
                                  " addr" & integer'image(i), Clk, Addr, RdData, RdEccSec, RdEccDed);
                    end loop;
                end loop;
