...
standalone entity, so it is also accessible from Verilog (which cannot call VHDL package functions).

Single-bit errors (SEC) are corrected before _Out_Data_ is driven. Double-bit errors (DED) are detected via
