...
architecture rtl of olo_ft_ram_sp_scrub is
    ...
    -- Scrubber-driven request signals
    signal Scrub_Rd_Ena   : std_logic;
    signal Scrub_Rd_Addr  : std_logic_vector(AddrWidth_c - 1 downto 0);
    signal Scrub_Wr_Ena   : std_logic;
    signal Scrub_Wr_Addr  : std_logic_vector(AddrWidth_c - 1 downto 0);
    signal Scrub_Wr_Data  : std_logic_vector(Width_g - 1 downto 0);
    -- Pulses on the cycle the scrubber's own read returns through the codec; used to mask the
    -- user-facing RdValid so scrubber-owned read cycles do not pulse it.
    signal Scrub_Rd_Valid : std_logic;

    -- Combined "port is busy this cycle" for the scrubber. Single-port RAM:
    -- user wins for both reads and writes; scrubber only acts on truly idle
    -- cycles. The scrubber's collision detection still uses `WrEna` alone.
...
begin
    ...
    -- Write-path mux (mutually exclusive thanks to Scrub_Wr_Ena gating on User_PortBusy).
    -- ErrInj_BitFlip / ErrInj_Valid pass straight through to the codec's latch -- see the
    -- "Error Injection" note in the doc for the interaction with scrubber writebacks.
    Ram_Addr   <= Addr   when User_PortBusy = '1' else Scrub_Wr_Addr when Scrub_Wr_Ena = '1' else Scrub_Rd_Addr;
    ...
    i_scrubber : entity work.olo_ft_ram_scrubber
        ...
        port map (
            ...
            User_Wr_Ena      => WrEna,
            User_Wr_Addr     => Addr,
            User_Wr_PortBusy => User_PortBusy,
            User_Rd_PortBusy => User_PortBusy,
    ...
    RdEccSec    <= Dec_RdEccSec;
    RdEccDed    <= Dec_RdEccDed;
    RdValid     <= Ram_RdValid and not Scrub_Rd_Valid;
    Scrub_Valid <= Scrub_Rd_Valid;
