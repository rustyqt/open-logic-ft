...
architecture rtl of olo_ft_ram_sdp_scrub is
    ...
    -- user-facing Rd_Valid so scrubber-owned read cycles do not pulse it.
    signal Scrub_Rd_Valid : std_logic;

    -- Muxed inputs to the inner olo_ft_ram_sdp. Write and read ports are independent (dual port);
    -- the scrubber asserts its own Wr/Rd_Ena only when the corresponding user *_Ena = '0'.
...
begin
    ...
    -- decoded value is re-encoded with fresh parity by the inner codec.
    ...
    i_scrubber : entity work.olo_ft_ram_scrubber
        ...
        port map (
            ...
            User_Wr_PortBusy => Wr_Ena,
            User_Rd_PortBusy => Rd_Ena,
    ...
    Rd_EccSec   <= Dec_Rd_EccSec;
    Rd_EccDed   <= Dec_Rd_EccDed;
    Rd_Valid    <= Ram_Rd_Valid and not Scrub_Rd_Valid;
